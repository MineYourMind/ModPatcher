		...
		while (c != null) {
			for (Field field : c.getDeclaredFields()) {
				...
				try {
					...
					PatcherLog.warn("Exception cleaning " + o.getClass() + '@' + System.identityHashCode(o), e);
				}
			}
			c = c.getSuperclass();
		}
	...
	public static void stopThread(Thread t, Throwable thr) {
		try {
			...
			try {
				...
				PatcherLog.error("Failed to stop thread t with throwable, reverting to normal stop.", throwable);
				t.stop();
			}
		}
	}
